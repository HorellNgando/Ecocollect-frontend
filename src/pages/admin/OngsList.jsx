...
// import { 
//   FiEdit2, FiPlus, FiHeart, FiMail, FiPhone, FiMapPin, 
//   FiCalendar, FiCheckCircle, FiXCircle, FiSearch, FiFilter,
//   FiDownload, FiRefreshCw, FiChevronLeft, FiChevronRight,
//   FiMoreVertical, FiAward, FiUsers, FiGlobe, FiFileText,
//   FiEye, FiUser, FiBriefcase, FiHome
// } from 'react-icons/fi';
// import { FaHandHoldingHeart, FaRegBuilding, FaHeartbeat } from 'react-icons/fa';
...
//       console.log('Réponse API ongs:', response);
      
//       let ongData = [];
//       if (response && response.ongs) {
//         ongData = response.ongs;
//       } else if (Array.isArray(response)) {
//         ongData = response;
//       }
      
//       // Enrichir avec des données supplémentaires pour la démo
//       const enrichedData = ongData.map((ong, index) => ({
//         ...ong,
//         domaine: ['Environnement', 'Social', 'Éducation', 'Santé'][index % 4],
//         zone: ['Dakar', 'Pikine', 'Guédiawaye', 'Rufisque', 'Thiès'][index % 5],
//         telephone: ong.telephone || '+221 77 123 45 6' + (index + 1),
//         adresse: ong.adresse || '123 Rue Principale, ' + ['Dakar', 'Pikine', 'Guédiawaye'][index % 3],
//         date_creation: ong.date_creation || '2020-01-0' + (index + 1),
//         projets_actifs: Math.floor(Math.random() * 10) + 1,
//         membres_actifs: Math.floor(Math.random() * 50) + 10,
//         site_web: ong.site_web || `www.ong${index + 1}.org`,
//         description: ong.description || 'ONG engagée pour le développement durable et la protection de l\'environnement.'
//       }));
      
//       setOngs(enrichedData);
//     } catch (error) {
//       console.error(error);
...
//   const domains = [...new Set(ongs.map(o => o.domaine).filter(Boolean))];

//   const filteredOngs = ongs.filter(ong => {
//     const matchesSearch = 
//       (ong.nom_ong?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
//       (ong.email?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
//       (ong.telephone || '').includes(searchTerm) ||
//       (ong.domaine?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
//       (ong.zone?.toLowerCase() || '').includes(searchTerm.toLowerCase());
    
//     const matchesStatus = filterStatus === 'all' || 
//       (filterStatus === 'actif' && ong.est_actif) || 
//       (filterStatus === 'inactif' && !ong.est_actif);
    
//     const matchesDomain = filterDomain === 'all' || ong.domaine === filterDomain;
...
//   const currentItems = filteredOngs.slice(indexOfFirstItem, indexOfLastItem);
//   const totalPages = Math.ceil(filteredOngs.length / itemsPerPage);

//   // Statistiques
//   const stats = {
//     total: ongs.length,
//     actifs: ongs.filter(o => o.est_actif).length,
//     inactifs: ongs.filter(o => !o.est_actif).length,
//     totalProjets: ongs.reduce((sum, o) => sum + (o.projets_actifs || 0), 0),
//     totalMembres: ongs.reduce((sum, o) => sum + (o.membres_actifs || 0), 0)
//   };
...
//     };
//     return colors[domaine] || 'bg-purple-100 text-purple-800 border-purple-200';
//   };

//   if (loading) {
//     return (
//       <AdminLayout title="ONG">
...
//                 <FaHeartbeat className="w-8 h-8 text-emerald-500" />
//               </div>
//             </div>
//             <p className="mt-4 text-gray-600 font-medium">Chargement des ONG...</p>
//           </div>
//         </div>
//       </AdminLayout>
//     );
...
//   return (
//     <AdminLayout title="ONG" user={user}>
//       <div className="p-6 bg-gray-50 min-h-screen">
//         {/* En-tête avec gradient */}
//         <div className=" bg-emerald-600 from-rose-500 to-rose-700 rounded-2xl p-8 mb-8 text-white shadow-xl">
...
//               </h1>
//               <p className="text-rose-100">Gérez les ONG partenaires et suivez leurs activités sur la plateforme</p>
//             </div>
...
//               </button>
//               <Link 
//                 to="/admin/ongs/nouveau" 
//                 className="inline-flex items-center gap-2 bg-white text-emerald-600 px-6 py-2 rounded-xl font-semibold hover:bg-emerald-100 transition-colors shadow-lg"
//               >
...
//               <p className="text-sm text-gray-500">Total ONG</p>
//               <div className="p-2 bg-rose-100 rounded-lg">
//                 <FaHeartbeat className="w-4 h-4 text-rose-600" />
//               </div>
//             </div>
//             <p className="text-2xl font-bold text-gray-900">{stats.total}</p>
//             <p className="text-xs text-gray-400 mt-1">Organisations</p>
//           </div>
...
//           <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-lg transition-shadow">
//             <div className="flex items-center justify-between mb-2">
//               <p className="text-sm text-gray-500">Domaines</p>
//               <div className="p-2 bg-yellow-100 rounded-lg">
//                 <FiGlobe className="w-4 h-4 text-yellow-600" />
//               </div>
//             </div>
//             <p className="text-2xl font-bold text-yellow-600">{domains.length}</p>
//             <p className="text-xs text-gray-400 mt-1">Secteurs d'activité</p>
//           </div>
//         </div>
...
//               <input
//                 type="text"
//                 placeholder="Rechercher par nom, email, téléphone, domaine ou zone..."
//                 value={searchTerm}
...
//                   className="pl-10 pr-8 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500 appearance-none bg-white min-w-[150px]"
//                 >
...
//                   <button onClick={() => setSearchTerm('')} className="ml-1 hover:text-rose-900">
...
//                   <button onClick={() => setFilterStatus('all')} className="ml-1 hover:text-rose-900">
//                     <FiXCircle className="w-4 h-4" />
//                   </button>
//                 </span>
//               )}
//               {filterDomain !== 'all' && (
//                 <span className="inline-flex items-center gap-1 px-3 py-1 bg-rose-50 text-rose-700 rounded-full text-sm">
//                   Domaine: {filterDomain}
//                   <button onClick={() => setFilterDomain('all')} className="ml-1 hover:text-rose-900">
//                     <FiXCircle className="w-4 h-4" />
//                   </button>
//                 </span>
//               )}
...
//                 <tr>
//                   <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">ONG</th>
...
//                   <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Domaine</th>
//                   <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Agrément</th>
...
//                 {currentItems.map((ong) => (
//                   <tr key={ong.id} className="hover:bg-gray-50 transition-colors group">
//                     <td className="px-6 py-4">
//                       <div className="flex items-center">
//                         <div className="w-10 h-10 bg-gradient-to-br from-emerald-100 to-rose-50 rounded-xl flex items-center justify-center shadow-sm group-hover:shadow transition-shadow">
//                           <FaHeartbeat className="w-5 h-5 text-emerald-600" />
//                         </div>
//                         <div className="ml-3">
//                           <div className="text-sm font-semibold text-gray-900">{ong.nom_ong}</div>
...
//                         <span className="text-sm text-gray-900">{ong.nom_responsable || 'Non spécifié'}</span>
//                       </div>
//                     </td>
//                     <td className="px-6 py-4">
...
//                           <span className="truncate max-w-[150px]">{ong.email}</span>
//                         </div>
//                         <div className="flex items-center gap-2 text-xs text-gray-600">
//                           <FiPhone className="w-3.5 h-3.5 text-gray-400" />
//                           <span>{ong.telephone}</span>
//                         </div>
//                       </div>
//                     </td>
//                     <td className="px-6 py-4">
//                       <span className={`px-2 py-1 text-xs font-medium rounded-lg ${getDomainColor(ong.domaine)}`}>
//                         {ong.domaine || 'Non spécifié'}
//                       </span>
//                     </td>
//                     <td className="px-6 py-4">
//                       <div className="flex items-center gap-1">
//                         <FiFileText className="w-3.5 h-3.5 text-gray-400" />
//                         <span className="text-sm text-gray-900">{ong.numero_agrement || '-'}</span>
//                       </div>
//                     </td>
                    
//                     <td className="px-6 py-4">
//                       {getStatusBadge(ong.est_actif)}
//                     </td>
//                     <td className="px-6 py-4">
//                       <div className="flex items-center gap-2">
//                         <button
...
//                         <div className="w-16 h-16 bg-emerald-300 rounded-full flex items-center justify-center mb-3">
//                           <FaHeartbeat className="w-8 h-8 text-gray-400" />
//                         </div>
//                         <p className="text-gray-500 font-medium">Aucune ONG trouvée</p>
...
//                   {Math.min(indexOfLastItem, filteredOngs.length)}
//                 </span>{' '}
//                 sur <span className="font-medium">{filteredOngs.length}</span> ONG
//               </div>
//               <div className="flex items-center gap-2">
//                 <button
...
//                 </button>
//                 <span className="px-4 py-2 bg-emerald-200 text-emerald-700 rounded-lg font-medium">
...
//               {/* En-tête du modal */}
//               <div className="bg-emerald-600 from-rose-600 to-rose-700 p-6 rounded-t-2xl">
...
//                     </div>
//                     <div>
//                       <h3 className="text-2xl font-bold">{selectedOng.nom_ong}</h3>
...
//                   <div className="bg-gray-50 p-4 rounded-xl">
//                     <h4 className="text-sm font-medium text-gray-500 mb-3 flex items-center gap-2">
//                       <FiBriefcase className="w-4 h-4" /> Informations
//                     </h4>
//                     <div className="space-y-2">
//                       <p className="flex justify-between">
//                         <span className="text-gray-600">Responsable:</span>
//                         <span className="font-medium text-gray-900">{selectedOng.nom_responsable || 'Non spécifié'}</span>
//                       </p>
//                       <p className="flex justify-between">
//                         <span className="text-gray-600">Domaine:</span>
//                         <span className={`px-2 py-1 text-xs font-medium rounded-lg ${getDomainColor(selectedOng.domaine)}`}>
//                           {selectedOng.domaine || 'Non spécifié'}
//                         </span>
//                       </p>
                      
//                       <p className="flex justify-between">
//                         <span className="text-gray-600">Agrément:</span>
//                         <span className="font-medium text-gray-900">{selectedOng.numero_agrement || '-'}</span>
//                       </p>
//                     </div>
//                   </div>
//                 </div>
...
//                   </h4>
//                   <p className="text-gray-900">{selectedOng.adresse || 'Non spécifiée'}</p>
//                 </div>
...
//                       {new Date(selectedOng.cree_le).toLocaleDateString('fr-FR', {
//                         day: '2-digit',
//                         month: '2-digit',
//                         year: 'numeric'
//                       })}
...
//                   </button>
//                   <Link
//                     to={`/admin/ongs/${selectedOng.id}/edit`}
    ...
    <AdminLayout title="Sponsors" user={user}>
      <div className="p-6 bg-gray-50 min-h-screen">
        ...
        {showModal && selectedSponsor && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
              {/* En-tête du modal avec gradient emerald */}
                ...
                <div className="flex items-center justify-between text-white">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-emerald-100">Sponsor • ID: {selectedSponsor.id}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setShowModal(false)}
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-emerald-50 p-4 rounded-xl">
                    <p className="text-sm text-emerald-600 mb-1">Montant total</p>
                    <p className="text-2xl font-bold text-emerald-700">{formatMontant(selectedSponsor.montant_total)}</p>
                  </div>
                  <div className="bg-emerald-50 p-4 rounded-xl">
                    <p className="text-sm text-emerald-600 mb-1">Contributions</p>
                    <p className="text-2xl font-bold text-emerald-700">{selectedSponsor.contributions}</p>
                  </div>
                  <div className="bg-emerald-50 p-4 rounded-xl">
                    <p className="text-sm text-emerald-600 mb-1">Projets soutenus</p>
                    <p className="text-2xl font-bold text-emerald-700">{selectedSponsor.projets_soutenus}</p>
                  </div>
                </div>
